...
public:
    ...
    submit(const std::filesystem::path& path,
           std::span<const CheckpointReadRequest> requests) = 0;
};
...
public:
    ...
    lock_directory(const std::filesystem::path& path) = 0;
    ...
    virtual void write_payloads_durable(
        const std::filesystem::path& path, std::span<const CheckpointPayload> payloads,
        std::span<const std::uint64_t> payload_offsets, std::uint64_t total_bytes) = 0;
