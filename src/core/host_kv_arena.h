...
// arena - bounds the largest session a configuration can admit back. Exposed as free functions so
// the bound is testable without a device, and used by both the export gate and the startup
// summary: a pool smaller than a session's KV turns a reported save into a session that answers
