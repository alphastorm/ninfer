...
void retire_generation_locked(const SessionCheckpointStoreOptions& options,
    ...
    try {
        ...
        const std::filesystem::path tombstone = unique_tombstone(
            options, digest + "--generation--" + std::string(generation));
...
SessionCheckpointEraseResult SessionCheckpointManager::erase_response(
    ...
    try {
        const AuthenticatedCheckpointNamespace checkpoint_namespace =
            AuthenticatedCheckpointNamespace::authenticated(tenant_sha256_,
                                                            std::string(session_sha256));
        const ResponseStoreTransactionalEraseResult erased =
            responses.erase_for_session_transactionally(
                response_id, session_sha256,
