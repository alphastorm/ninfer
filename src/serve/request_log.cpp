    ...
    return Json{
        ...
        {"scheduler",
         Json{{"max_concurrency", options.max_concurrency},
              ...
              // This engine has no materialization or deferred-capture pipeline; zero is the
              // true count, and the pinned OMP client requires the concrete field (ninfer#28).
              {"materializing", 0},
              {"capture_pending", 0}}},
        {"cache",
         Json{{"prefix_reuse", options.allow_prefix_reuse},
              // Aggregate cache telemetry is not tracked by this engine build; the fields the
              // pinned OMP 18.0.9 client validates (private_catalog occupancy/capacity and
              // reused_prompt_tokens) are emitted as concrete floor values instead of null,
              // matching the v0.4.0 container precedent of concrete zeros (ninfer#28).
              {"device_state", Json{{"occupied", 0}, {"capacity", options.max_concurrency}}},
              {"host_state", Json{{"occupied", 0}, {"capacity", 0}}},
              {"host_kv", Json{{"occupied_bytes", 0}, {"capacity_bytes", 0}}},
              {"private_catalog", Json{{"occupied", 0}, {"capacity", 0}}},
              {"shared_catalog", Json{{"occupied", 0},
                                       {"capacity", 0},
                                       {"active_references", 0}}},
              {"reused_prompt_tokens", 0},
              {"last_selection", Json{{"path", "root"}, {"frontier_tokens", 0}}},
              {"private_evictions", 0},
              {"shared_evictions", 0},
              {"last_materialization_predicted_nanoseconds", 0},
        ...
        {"mtp",
         Json{{"enabled", options.speculative.backend == ninfer::SpeculativeBackend::Mtp},
              // Per-request MTP counters are logged on completion lines; aggregate counters
              // are not tracked by this engine build. The pinned client requires concrete
              // non-negative counts, so the untracked aggregates are emitted as zero and the
              // ratio fields stay null exactly like the container lane before its first round.
              {"rounds", 0},
              {"drafted_tokens", 0},
              {"accepted_tokens", 0},
              {"fallback_steps", 0},
