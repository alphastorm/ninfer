...
def json_response(
    ...
    *,
    ...
    if response.content_type != "application/json":
        raise ContractError(
            f"{method} {path} returned content-type={response.content_type}"
        )
    try:
...
def configure_api_key(path: Path | None) -> None:
    ...
    try:
        ...
        raise ContractError("failed to read --api-key-file") from error
    if not value or b"\n" in value or b"\r" in value or b"\x00" in value:
        raise ContractError("--api-key-file must contain one non-empty line")
    try:
        api_key = value.decode("utf-8")
    except UnicodeDecodeError as error:
        raise ContractError("--api-key-file must contain UTF-8") from error
    _DEFAULT_HEADERS = {"Authorization": f"Bearer {api_key}"}
...
def exercise(base_url: str, model: str) -> dict[str, Any]:
    ...
    if content != expected_content:
        raise ContractError(
            "streamed content differs from the non-streaming greedy response"
        )
    if reasoning != expected_reasoning:
        raise ContractError(
            "streamed reasoning differs from the non-streaming greedy response"
        )
    ...
    streamed_text, streamed_reasoning, response_stream_terminal = (
        parse_responses_stream(response_stream)
    )
    if (streamed_text, streamed_reasoning) != (
        ...
        raise ContractError(
            "Responses streaming output differs from greedy non-streaming output"
        )
    _, streamed_output_tokens = require_responses_usage(
...
def main() -> None:
    ...
    configure_api_key(args.api_key_file)
