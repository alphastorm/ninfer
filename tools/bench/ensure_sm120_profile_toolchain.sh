...
image_provenance_matches() {
  local image_id=$1
  local base_label version_label source_label
...
image_matches() {
  local image_id revision_label
  image_id=$(docker_cli image inspect "$PROFILE_TOOLCHAIN_IMAGE" --format '{{.Id}}' 2>/dev/null) || return 1
  image_provenance_matches "$image_id" || return 1
  revision_label=$(docker_cli image inspect "$image_id" --format '{{index .Config.Labels "org.ninfer.profile.revision"}}' 2>/dev/null) || return 1
  [[ $revision_label == "$profile_image_revision" ]] || return 1
  ...
  docker_cli run --rm --pull never --network none --entrypoint /bin/bash "$image_id" -c \
    'set -e; for command in cmake ninja nvcc ncu nsys git sha256sum python3.11; do command -v "$command" >/dev/null; done; importer=$(find /opt/nvidia -type f -name QdstrmImporter -print -quit); test -n "$importer"; "$importer" --version >/dev/null; python3.11 -c "import sys; assert sys.version_info[:2] == (3, 11)"' \
    >/dev/null || return 1
  printf '%s\n' "$image_id"
}
...
existing_image_id=$(docker_cli image inspect "$PROFILE_TOOLCHAIN_IMAGE" --format '{{.Id}}' 2>/dev/null || true)
if [[ -n $existing_image_id ]] && ! image_provenance_matches "$existing_image_id"; then
...
docker_cli build --pull=false --file "$dockerfile" --tag "$PROFILE_TOOLCHAIN_IMAGE" \
  ...
  --build-arg "PROFILE_IMAGE_REVISION=$profile_image_revision" \
