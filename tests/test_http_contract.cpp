...
int main() {
    ...
    const auto status_result =
        ninfer::serve::authorize_http_request(open, status, status_response);
    ...
    failures += check(
        ninfer::serve::authorize_http_request(protected_options, ordinary, missing_response) ==
    ...
    failures += check(
        ninfer::serve::authorize_http_request(protected_options, bearer, bearer_response) ==
                HandlerResponse::Unhandled &&
    ...
    failures += check(
        ninfer::serve::authorize_http_request(protected_options, anthropic, anthropic_response) ==
            HandlerResponse::Unhandled,
    ...
    failures += check(wrong_anthropic_result == HandlerResponse::Handled &&
                          ...
                          wrong_anthropic_body.at("error").at("type") ==
    ...
    failures += check(
        ninfer::serve::authorize_http_request(protected_options, preflight, preflight_response) ==
            HandlerResponse::Unhandled,
    ...
    const auto payload_result = ninfer::serve::handle_unrendered_http_error(
        payload_options, ordinary, payload_response);
    ...
    authored_response.set_content(R"({"error":{"code":"application_error"}})",
                                  "application/json");
    const std::string authored_body = authored_response.body;
    failures += check(
        ninfer::serve::handle_unrendered_http_error(payload_options, ordinary, authored_response) ==
                HandlerResponse::Unhandled &&
    ...
    ninfer::serve::GenerationRequest merged_identity;
    ninfer::serve::apply_response_session_identity(session_request, true, merged_identity);
    failures += check(merged_identity.client_session_sha256 == session_digest,
                      "Responses header identity was not applied to the request body");
    ninfer::serve::GenerationRequest conflicting_identity;
    conflicting_identity.client_session_sha256 = std::string(64, 'b');
    std::string conflicting_code;
    try {
        ninfer::serve::apply_response_session_identity(session_request, true,
                                                       conflicting_identity);
    } catch (const ninfer::serve::ApiException& exception) {
        conflicting_code = exception.error().code;
    }
    failures += check(conflicting_code == "invalid_ninfer_identity",
                      "conflicting Responses body/header sessions were accepted");
