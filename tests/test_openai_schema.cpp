...
int test_parse_tool_history_messages() {
    ...
    failures += check(string_req.messages[1].tool_calls[0].name == "get_weather",
    ...
    failures += check(string_req.messages[2].role == "tool", "tool role parsed");
    ...
    failures += check(to_request_options(string_req, default_server()).output.preserve_special_tokens,
